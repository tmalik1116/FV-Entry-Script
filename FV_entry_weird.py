...
def run(filename): # currently set to read CSV, can adjust to detect file type
    ...
    for i in range(1, len(data)): # item is a row
        ...
        ### enhanced version (only for my format) ###
        mouse.move(0, 115)
        time.sleep(0.25)
        mouse.click(Button.left, 1)
        mouse.move(155, 100)
        ...
        print(data[i])
        print(data[i][2])
        cc_location(data[i]) # selects location
        ...
        mouse.move(0, 180)
        mouse.click(Button.left, 1)
        ### END OF LOCATION INSERT (REMOVE THIS) ###
...
# locations = ["br", "lib", "cc"]
filenames = ["basement_detail.csv", "ground_detail.csv", "second_detail.csv", "third_detail.csv"]

# running for each file
for i in range(4):
    run(filenames[i])

# run("third_detail.csv")